...
def get_extensions():
    ...
    if os.getenv('MMCV_WITH_TRT', '0') != '0':
        ...
        # Add standard and the flag to allow unsupported compiler
        extra_compile_args['nvcc'] += ['-std=c++14', '-allow-unsupported-compiler']
    ...
    if EXT_TYPE == 'parrots':
        ...
        cuda_args = os.getenv('MMCV_CUDA_ARGS')
        extra_compile_args = {
            'nvcc': [cuda_args, '-std=c++14', '-allow-unsupported-compiler'] if cuda_args else ['-std=c++14', '-allow-unsupported-compiler'],
            'cxx': ['-std=c++14'],
        }
