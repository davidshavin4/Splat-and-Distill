...
class DepthEncoderDecoder(BaseDepther):
    ...
    def __init__(
        ...
        self.eval_vggt = False
        if self.eval_vggt:
            print('WARNING: eval_vggt is set to True in DepthEncoderDecoder, this should only be for training vggt')
        ...
        # print("WARNING!!!! enablnig grad, this should only be for vggt")
        # for p in self.vggt.depth_head.parameters():
    ...
    def forward_train(self, img, img_metas, depth_gt, **kwargs):
        ...
        """
        if self.eval_vggt: #when training vggt this should be set to true
            return self.backbone.get_intermediate_layers(img, depth_gt=depth_gt)
        x = self.extract_feat(img)
    ...
    def whole_inference(self, img, img_meta, rescale, size=None):
        ...
        if self.eval_vggt:
            return self.backbone.get_intermediate_layers(img, eval=True)
