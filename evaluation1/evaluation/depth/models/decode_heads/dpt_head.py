    ...
    def __init__(
        ...
        if self.readout_type == "project":
            ...
            for _ in range(len(self.projects)):
                self.readout_projects.append(
                    nn.Sequential(
                        Linear(in_channels + int(cls_factor * in_channels), in_channels),
                        build_activation_layer(dict(type="GELU")),
                    )
                )
    ...
    def forward(self, inputs):
        ...
        for i, x in enumerate(inputs):
            ...
            if self.readout_type == "project":
                ...
                readout = cls_token[:, None, :].expand_as(x[:,:,:cls_dim])
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.reassemble_blocks = ReassembleBlocks(
            embed_dims, post_process_channels, readout_type, patch_size, cls_factor
        )
