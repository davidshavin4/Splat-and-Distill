...
class NYUDataset(Dataset):
    ...
    def load_annotations(self, data_root, split):
        ...
        self.running_vggt = False
        if self.running_vggt:
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print("WARNING, this should run only when running VGGT backbone, i'm in evaluation.depth.datasets.nyu.py")            
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print_log(
                "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\nWARNING, this should run only when running VGGT backbone, i'm in evaluation.depth.datasets.nyu.py\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!",                   
                logger=get_root_logger(),
            )
        return img_infos
    ...
    def prepare_train_img(self, idx):
        ...
        if self.running_vggt:
            ...
            output['img']._data = F.interpolate(output['img'].data[None], size=(518, 518), mode='bilinear', align_corners=False)[0].to(torch.uint8)
            output['depth_gt']._data = F.interpolate(output['depth_gt'].data[None], size=(518, 518), mode='bilinear', align_corners=False)[0]
        return output
    ...
    def pre_eval(self, preds, indices):
        ...
        for i, (pred, index) in enumerate(zip(preds, indices)):
            ...
            if self.running_vggt:
                import torch.nn.functional as F
                depth_map_gt = F.interpolate(torch.from_numpy(depth_map_gt)[None], size=(518, 518), mode='bilinear', align_corners=False)[0].numpy()
