...
class ScanNetPPDepthDataset(Dataset):
    ...
    def pre_eval(self, preds, indices):
        ...
        for i, (pred, index) in enumerate(zip(preds, indices)):
            ...
            # target_h, target_w = 584//4, 876//4
            target_h, target_w = 584, 876
            depth_map_gt_t = torch.from_numpy(depth_map_gt).unsqueeze(0).unsqueeze(0)  # (1,1,H,W)
            depth_map_gt_t = torch.nn.functional.interpolate(
                depth_map_gt_t,
                size=(target_h, target_w),
                mode="bilinear",
                align_corners=False,
            )
            depth_map_gt = depth_map_gt_t.squeeze(1).numpy() 

            # depth_map_gt = np.expand_dims(depth_map_gt, axis=0)
            if depth_map_gt.shape != pred.shape:
                pred = torch.nn.functional.interpolate(
                    torch.tensor(pred).permute(0,3,1,2),
                    depth_map_gt.shape[1:],
                    mode="bilinear",
                    align_corners=False,
