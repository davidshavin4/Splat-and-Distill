...
from evaluation.segmentation.apis import train_segmentor
from evaluation.segmentation.datasets import build_dataset
from evaluation.eval_utils.misc import create_segmentor
from mmcv.utils import Config, DictAction
...
def main(args):
    ...
    if args.work_dir is not None:
        ...
        cfg.work_dir = osp.join(args.work_dir, timestamp)
    ...
    log_file = osp.join(cfg.work_dir, "logger.log")    
    ...
    # log some basic info
    logger.info(f"model name: {args.backbone_type}")
    ...
    if args.eval_baseline:
        ...
        vit = vit.to(device)
        backbone_model = vit
        logger.info(f"path:  {backbone_model.path}")
    else:
        fit3d = FiT3D(args.backbone_type)
        fit3d.eval()
        fit3d.to(device)
        backbone_model = fit3d
        logger.info(f"path:  {backbone_model.finetuned_model.path}")
    ...
    model = create_segmentor(cfg, backbone_model=backbone_model)
